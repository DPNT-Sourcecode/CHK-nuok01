...
class CheckoutSolution {
    ...
    checkout(skus) {
        ...
        let groupItems = [];
        for (let sku of groupSKUs) {
            if (counts[sku]) {
                for (let i = 0; i < counts[sku]; i++) {
                    groupItems.push(sku);
                }
                counts[sku] = 0;
            }
        ...
        let groupTotal = 0;
        const numberOfgroups = Math.floor(groupItems.length / 3);
        groupTotal += numberOfgroups * 45;

        const leftoverStart = numberOfgroups * 3;
        for (let i = leftoverStart; i < groupItems.length; i++) {
            groupTotal += prices[groupItems[i]];
        }
